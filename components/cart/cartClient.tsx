      ...
      <div className=" mx-auto max-w-screen-xl px-1 py-8 sm:px-6 sm:py-12 lg:px-8">
        <div className="mx-auto max-w-3xl">
          <header className="text-center">
            <h1 className="text-2xl font-bold font-[Cormorant] text-myblack sm:text-3xl">
              Votre Panier
            </h1>
          </header>
          {cartItems &&
                  ...
                  <ul key={item.id} className="space-y-4">
                    <li className="flex items-center gap-4">
                      ...
                      <div>
                        {item.categoryId === 1 ? (
                          <Link
                            className="text-sm sm:text-xl ml-0 text-redhot"
                            href={`vins/${item.id}`}
                          >
                            {item.name}
                          </Link>
                          ...
                          <Link
                            className="text-sm sm:text-xl ml-0 text-redhot"
                            href={`champagnes/${item.id}`}
                          >
                            {item.name}
                          </Link>
                        ) : (
                          <Link
                            className="text-xs sm:text-xl ml-0 text-redhot"
                            href={`spiritueux/${item.id}`}
                          >
                            {item.name}
                          </Link>
                        )}
                      ...
                      <div className="flex flex-1 items-center justify-end gap-2">
                        <div className="flex items-center py-1 gap-1 max-sm:size-8 max-sm:-ml-8 ">
                          <button
                            type="button"
                            className="size-10 max-sm:size-4 max-sm:-bottom-5 max-sm:left-[11px] max-sm:justify-self-center leading-10 text-gray-400 transition hover:opacity-75"
                          ...
                          <button
                            type="button"
                            className="size-10 max-sm:size-4 max-sm:-top-5 max-sm:right-[29px] max-sm:justify-self-center leading-10 text-gray-400 transition hover:opacity-75"
                        ...
                        <p className="mt-0.5 space-y-px sm:text-sm font-semibold text-xs max-sm:w-10 text-gray-600">
                          {item.soldPrice &&
                            item.quantity &&
                            formatPrice(item.soldPrice * item.quantity)}{" "}
                          FCFA
                        </p>
            ...
            <div className="w-screen max-w-lg space-y-4">
              <dl className="space-y-0.5 text-sm text-gray-700">
                ...
                <div className="flex justify-between !text-base font-medium">
                  <dt>Total</dt>
                  <dd>
                    {formatPrice(cartTotalAmount + cartTotalAmount * 0.18 - Discount)} FCFA
                  </dd>
                </div>
              </dl>
              ...
              <div className="flex justify-end">
                <span className="inline-flex items-center justify-center rounded-full bg-red200 px-2.5 py-0.5 text-pourpre">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
                    viewBox="0 0 24 24"
                  ...
                  <p className="whitespace-nowrap text-xs">
                    0 Bon(s) de réduction appliqué(s)
                  </p>
                </span>
              </div>
              ...
              <div className="flex justify-end gap-x-5">
                <Button
                  ...
                  variant="outline"
                  className="h-12 text-pourpre hover:bg-red200"
                >
                  Vider le panier
                </Button>
                <PaymentPage />
